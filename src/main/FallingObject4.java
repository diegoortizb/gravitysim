...
public class FallingObject4 extends FallingObject {
	...
	static float bounce = .4f;
	static float gravity = .2f;
	static int bottom = Sim.HEIGHT - 40;

	public FallingObject4(float x, float y, ID id, Handler handler) {
		super(x, y, id);
	...
	public void collision() {
		for (int i = 0; i < handler.object.size(); i++) {
			...
			if (tempObject.getId() != ID.FallingObject4) {
